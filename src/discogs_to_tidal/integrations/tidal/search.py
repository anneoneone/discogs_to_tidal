...
from ...core.models import Track, Album
...
class TidalSearchService:
    ...
    def find_tracks_by_album(
        ...
        for i, track in enumerate(tracks, 1):
            artist_name = (track.primary_artist.name
                           if track.primary_artist else 'Unknown')
            logger.info(
    ...
    def _match_track_in_album(
        self,
        discogs_track: Track,
        tidal_tracks: List[tidalapi.Track]
        ...
        discogs_artist = normalize_string(
            self._clean_artist(discogs_track.primary_artist.name)
        ) if discogs_track.primary_artist else ""
        ...
        for tidal_track in tidal_tracks:
            ...
            position_bonus = 0
            if (discogs_track.track_number and
                    hasattr(tidal_track, 'track_num') and
                    tidal_track.track_num == discogs_track.track_number):
                position_bonus = 0.1
        ...
        if best_match:
            logger.debug(f"    Matched: {discogs_track.title} -> {best_match.name} "
    ...
    def _merge_album_tracks(
        ...
        """
        logger.debug(f"Merging {len(individual_results)} individual results with "
    ...
    def _clean_artist(self, artist: str) -> str:
        ...
        cleaned = re.sub(r'\s+(feat\.?|featuring|ft\.?|f\.)\s+.*$', '', cleaned,
                         flags=re.IGNORECASE)
    ...
    def _create_track_conversion_data(
        self,
        discogs_track: Track,
        tidal_track: Optional[tidalapi.Track]
    ) -> Dict[str, Any]:
